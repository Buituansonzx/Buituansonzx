...
public class SignupController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user.length() < 5 || user.length() > 50) {
            request.setAttribute("mess", "Username must be between 5 and 50 characters.");
            request.getRequestDispatcher("login.jsp").forward(request, response);
        }else if(!pass.matches(".*[A-Z].*") || !pass.matches(".*\\d.*")){
            request.setAttribute("mess", "Password must contain at least one uppercase letter and one digit.");
            request.getRequestDispatcher("login.jsp").forward(request, response);
        }else{
